...
import random
from utils.zoom_utils import create_zoom_meeting
from datetime import datetime, timedelta
import os
...
# app.config['MYSQL_PASSWORD'] = 'Mutthu3234#'
# app.config['MYSQL_DB'] = 'parul'


# # Mail setup (using Gmail SMTP)
...
def send_otp_email(email, otp, purpose='password_reset'):
    try:
        if purpose == 'registration':
            subject = 'Email Verification - Healthcare System'
            body = f"""
...
Healthcare Team
            ...
            subject = 'Heart Disease App - Password Reset OTP'
            body = f"Your OTP for password reset is {otp}. It will expire in 5 minutes."
        
        msg = Message(subject, sender='mrravikoya@gmail.com', recipients=[email])
        msg.body = body
        mail.send(msg)
        print(f"✅ {purpose.replace('_', ' ').title()} OTP sent to: {email}")
        return True
        
    except Exception as e:
        print(f"❌ Failed to send {purpose} OTP: {str(e)}")
        return False

def send_appointment_confirmation(email, appointment_data):
    try:
        # Format the date properly
        appointment_date = datetime.strptime(str(appointment_data['preferred_date']), '%Y-%m-%d').strftime('%B %d, %Y')
        
        msg = Message(
            subject='Appointment Confirmation - Healthcare System',
            sender='mrravikoya@gmail.com',
            recipients=[email]
        )
...
Thank you for choosing our Healthcare System. Your appointment has been successfully booked.
...
CONTACT INFORMATION:
───────────────────
📍 Hospital Address: 123 Medical Center, Healthcare City
📞 Phone: +1 (555) 123-HELP
📧 Email: support@healthcaresystem.com

We look forward to serving you and wish you the best in your health journey.
...
Healthcare System
        ...
        print(f"❌ Failed to send appointment email: {str(e)}")
        return False
...
def register():
    if request.method == 'POST':
        try:
            ...
            # Store OTP in password_resets table with 'registration' purpose
            cursor.execute("DELETE FROM password_resets WHERE email = %s", (email,))
            cursor.execute(
                "INSERT INTO password_resets (email, otp, expiry, purpose) VALUES (%s, %s, %s, 'registration')",
            ...
            if email_sent:
                return jsonify({'success': True, 'message': '✅ OTP sent to your email! Please verify to complete registration.'})
            else:
...
def forgot_password():
    if request.method == 'POST':
        ...
        # Generate OTP and store in table
        otp = str(random.randint(100000, 999999))
        ...
        cursor.execute("DELETE FROM password_resets WHERE email = %s", (email,))
        cursor.execute(
            "INSERT INTO password_resets (email, otp, expiry, purpose) VALUES (%s, %s, %s, 'password_reset')",
            (email, otp, expiry_time)
        )
        mysql.connection.commit()
        cursor.close()
...
def verify_otp():
    if request.method == 'POST':
        # Determine the purpose from session
        purpose = 'password_reset' if 'reset_email' in session else 'registration'
        email = session.get('reset_email') or session.get('reg_email')
        
        if not email:
            return jsonify({'success': False, 'message': '❌ Please complete the first step.'})

        entered_otp = request.form.get('otp')

        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT * FROM password_resets WHERE email = %s AND purpose = %s", (email, purpose))
        otp_record = cursor.fetchone()

        if not otp_record:
            return jsonify({'success': False, 'message': '❌ No OTP found. Please try again.'})

        if otp_record['otp'] != entered_otp:
            return jsonify({'success': False, 'message': '❌ Incorrect OTP!'})

        if datetime.now() > otp_record['expiry']:
            cursor.execute("DELETE FROM password_resets WHERE email = %s AND purpose = %s", (email, purpose))
            mysql.connection.commit()
            cursor.close()
    ...
    # For GET request - determine which template to show
    if 'reset_email' in session:
        return render_template('verify_otp.html', purpose='password_reset')
    elif 'reg_email' in session:
        return render_template('verify_otp.html', purpose='registration')
    else:
        flash("Please complete the first step.", "warning")
...
def reset_password():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("UPDATE eren SET password = %s WHERE email = %s", (hashed_pw, email))
        mysql.connection.commit()
        # Clean up OTP record
        cursor.execute("DELETE FROM password_resets WHERE email = %s", (email,))
        mysql.connection.commit()
...
def appointment():
    ...
    if request.method == 'POST':
        try:
            ...
            cursor.execute("SELECT full_name, specialization FROM doctor WHERE id = %s", (doctor_id,))
            ...
            email_sent = send_appointment_confirmation(email, appointment_data)

            if email_sent:
                flash("✅ Appointment booked successfully! Confirmation email sent.", "success")
            else:
                flash("✅ Appointment booked successfully! (Email notification failed)", "success")
...
def check_database_connection():
    try:
        ...
        # Check if password_resets table exists and add purpose column if needed
        cursor.execute("SHOW TABLES LIKE 'password_resets'")
        if not cursor.fetchone():
            print("❌ Password resets table doesn't exist. Creating it now...")
            cursor.execute("""
                CREATE TABLE password_resets (
                    ...
                    purpose ENUM('password_reset', 'registration') DEFAULT 'password_reset',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)
            print("✅ Password resets table created successfully")
        else:
            # Check if purpose column exists, if not add it
            cursor.execute("""
                SELECT COLUMN_NAME 
            ...
            """)
            if not cursor.fetchone():
                print("🔧 Adding purpose column to password_resets table...")
                cursor.execute("ALTER TABLE password_resets ADD COLUMN purpose ENUM('password_reset', 'registration') DEFAULT 'password_reset'")
                print("✅ Purpose column added successfully")
